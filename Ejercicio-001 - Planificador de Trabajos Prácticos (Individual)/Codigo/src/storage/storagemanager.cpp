...
StorageManager::StorageManager()
{
    ...
    dataDirectoryPathValue = basePath;
}
...
{
    return readLegacyUsersJsonFile(usersFilePath());
}

bool StorageManager::saveUsers(const QList<User> &users) const
{
    return writeUsersJsonFile(usersFilePath(), users);
}
...
{
    return QDir(dataDirectoryPathValue).filePath("users.json");
}

QString StorageManager::practicalWorksFilePath() const
{
    return QDir(dataDirectoryPathValue).filePath("practicalworks.txt");
}

QString StorageManager::legacyPracticalWorksJsonFilePath() const
{
    return QDir(dataDirectoryPathValue).filePath("practicalworks.json");
}
...
{
    return QDir(dataDirectoryPathValue).filePath("history.txt");
}

QString StorageManager::legacyHistoryJsonFilePath() const
{
    return QDir(dataDirectoryPathValue).filePath("history.json");
}

QString StorageManager::sessionFilePath() const
{
    return QDir(dataDirectoryPathValue).filePath("session.txt");
}

QString StorageManager::legacySessionJsonFilePath() const
{
    return QDir(dataDirectoryPathValue).filePath("session.json");
}

bool StorageManager::ensureDataDirectory() const
{
    QDir directory;
    return directory.mkpath(dataDirectoryPathValue);
}
...
bool StorageManager::ensureUsersFile() const
{
    ...
    const QString localLegacyTextPath = QDir(dataDirectoryPathValue).filePath("users.txt");
    if (QFile::exists(localLegacyTextPath)) {
        return writeUsersJsonFile(targetPath, readUsersTextFile(localLegacyTextPath));
    }

    const QString executableLegacyTextPath = QDir(executableDataDirectoryPath()).filePath("users.txt");
    if (QFile::exists(executableLegacyTextPath)) {
        return writeUsersJsonFile(targetPath, readUsersTextFile(executableLegacyTextPath));
    }
    ...
    if (QFile::exists(legacyJsonPath)) {
        ...
        if (!legacyUsers.isEmpty() || legacyFileInfo.size() == 0) {
            return writeUsersJsonFile(targetPath, legacyUsers);
        }
    }

    return ensureSeedFile(":/seed/users.json", targetPath);
}
...
bool StorageManager::writeUsersJsonFile(const QString &path, const QList<User> &users) const
{
    if (!ensureDataDirectory()) {
        return false;
    }
    ...
    const QJsonDocument document(usersArray);
